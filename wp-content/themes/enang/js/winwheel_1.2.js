...
var wheelImageName   = "http://localhost/vnup/wp-content/uploads/2015/09/spin.png";	// File name of the image for the wheel.
var spinButtonImgOn  = "http://localhost/vnup/wp-content/uploads/2015/09/spin_on.png";		// Name / path to the images for the spin button.
var spinButtonImgOff = "http://localhost/vnup/wp-content/uploads/2015/09/spin_off.png";
...
var power        = 2;  // Set when the power is selected. 1 for low, 2 for med, 3 for high.
...
function startSpin(determinedValue) {
	...
	if ((typeof(stopAngle) !== 'undefined') && (wheelState == 'reset') && (power))
	{
        ...
        //BEFORE DO SPIN check this email & type is existed on system?
        var data_check = {};
        data_check.username = userName;
        data_check.phone = phone;
        data_check.email = email;
        data_check.type_prize = type;
        data_check.before_spin = true;
        ...
        $.ajax({
            ...
            success: function(json) {
                if(json.status == 'Failed'){
                    alert('Vui long nhap email khac de tip tuc quay thuong');
                }
                else{
                    data_check.before_spin = false;
                    doSpin(data_check);
                }
            }
        });
			...
			for (x = 0; x < (prizes.length); x ++)
			{
				if ((relativeAngle >= prizes[x]['startAngle']) && (relativeAngle <= prizes[x]['endAngle']))
				{
                    ...
                    $.ajax({
                        ...
                        success: function(json) {
                            if(json.status == 'OK'){
                                alert("Chuc mung ban da nhan " + prizes[x]['name'] + "!\n Chung toi se lien lac voi ban som nhat de nhan thuong.");
                            }
                        }
                    });
					break;
